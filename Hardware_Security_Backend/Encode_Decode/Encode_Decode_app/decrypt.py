...
def InvCipher():
    global state, RoundKey, Nr
    
    for i in range(4):
        for j in range(4):
            state[j][i] = in_block[i * 4 + j]
    ...
    # Copy state to out_block after decryption
    for i in range(4):
        for j in range(4):
            out_block[i * 4 + j] = state[j][i]
...
def AES_decrypt(input_str, key_str):
